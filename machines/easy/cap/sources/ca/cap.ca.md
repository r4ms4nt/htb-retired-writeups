# HTB CAP — Explotació completa ordenada

> Document mestre reconstruït i ordenat a partir de les notes originals del cas.
>
> **Criteri aplicat:**
> - no s’elimina res del material original
> - es reorganitza el cas per fases
...
Per deixar aquest laboratori clar de cara al futur, convé fixar tres punts:

- **Vector inicial validat en aquest laboratori:** accés no autenticat al dashboard web i descàrrega de captures PCAP mitjançant una ruta numèrica (`/data/<id>`), amb una captura útil a `0.pcap`.
- **Pivot real validat:** extracció de credencials FTP en text clar des de `0.pcap` i reutilització correcta per **SSH** amb l’usuari `nathan`.
- **Escalada de privilegis validada en aquest laboratori:** abús de **Linux capabilities** sobre `/usr/bin/python3.8`, concretament `cap_setuid`, per elevar a `uid=0`.

### Nota editorial addicional

A les notes originals apareix aquesta frase:

> “Primero, revisamos el contenido de la carpeta home de nathan y vemos que hay un archivo llamado notes.txt.”

Tanmateix, a la sortida de `ls -la` conservada en el mateix material no apareix `notes.txt`.
Per no perdre el context històric, la frase original es conserva a l’annex, però en el document ordenat no es pren com a fet verificat.
...
2. Reconeixement inicial
3. Enumeració web i tancament de la fase 1
4. Descobriment del recurs PCAP útil
5. Anàlisi de `0.pcap` i extracció de credencials
6. Accés SSH com a `nathan`
7. Obtenció de `user.txt`
8. Enumeració local per a escalada de privilegis
9. Troballa clau: capabilities sobre Python
10. Escalada a root
11. Obtenció de `root.txt`
12. Resum tècnic final
13. Annex A — Notes originals íntegres

---

## 1. Preparació de l’entorn

### Creació del directori de treball
...
**Objectiu d’aquest pas:** crear l’espai de treball del cas i deixar separades les carpetes `content`, `exploits` i `nmap`.

### Connexió a la VPN d’HTB

```bash
sudo openvpn --config /home/r4mon/pentest/HTB-VPN/machines_eu-dedivip-1.ovpn
```

**Objectiu d’aquest pas:** connectar-se a la VPN de Hack The Box per poder interactuar amb la màquina objectiu.

### Fixar la IP objectiu

```bash
settarget 10.129.28.170 CAP
```

**Objectiu d’aquest pas:** deixar visible a l’entorn de treball la IP i el nom de la màquina objectiu.
...
**Objectiu d’aquest pas:** verificar que la màquina objectiu és accessible a través de la xarxa.

### Identificació ràpida del sistema
...
**Ports oberts observats:**

- `21/tcp` → FTP
- `22/tcp` → SSH
- `80/tcp` → HTTP

**Sortida rellevant conservada:**
...
- `80/tcp` → `gunicorn`
- títol web observat: **Security Dashboard**

**Interpretació útil:** la superfície dominant del cas passa a ser la **web**, perquè no només hi ha un port 80 obert, sinó una aplicació amb nom propi i backend identificable.

---

## 3. Enumeració web i tancament de la fase 1

Després d’accedir a `http://10.129.28.170/`, l’aplicació mostra un **dashboard de seguretat** i s’observa una sessió o context visual associat a **Nathan**.

En aquest moment, el tancament raonable de la fase 1 queda així:

- **superfície dominant:** web
- **branca principal:** `WEB-BASE`
- **branques secundàries:** FTP, SSH
- **següent pas únic:** perfilar la funcionalitat real del dashboard i revisar els recursos exposats per l’aplicació

Aquesta decisió es justifica perquè:

- la web retorna una aplicació real, no una landing buida;
- FTP i SSH encara no ofereixen accés útil per si mateixos;
- encara no s’havia confirmat cap credencial vàlida ni cap login reutilitzable.

---

## 4. Descobriment del recurs PCAP útil

Dins del dashboard s’accedeix a la funcionalitat:

```text
Security Snapshot (5 Second PCAP + Analysis)
```

La navegació porta a una ruta numerada:
...
La URL utilitza un identificador numèric i, a més, apareix un botó de descàrrega de captura.
Això suggereix que l’aplicació permet accedir a diferents PCAP segons l’ID.

### Prova realitzada

- descàrrega de `1.pcap`
- canvi manual de la URL cap a `http://10.129.28.170/data/0`
- descàrrega de `0.pcap`

### Resultat observat

- `1.pcap` és buit
- `0.pcap` conté trànsit útil

**Conclusió:** l’ID numèric és rellevant i la captura útil per al cas és `0.pcap`.

---

## 5. Anàlisi de `0.pcap` i extracció de credencials

El fitxer `0.pcap` s’obre amb **Wireshark**.

### Troballa clau

Dins la captura s’observa una sessió **FTP en text clar** amb aquestes credencials:
...
### Confirmació de validesa

La mateixa captura mostra:
...
Per tant, la credencial queda validada **com a mínim per a FTP**.

### Altres ordres visibles a la captura
...
### Observació rellevant

L’operació `RETR notes.txt` acaba amb error:

```text
550 Failed to open file.
```

Per tant, el valor real del PCAP no és aquest fitxer, sinó la **credencial reutilitzable** exposada en clar.

### Canvi correcte de branca

En aquest punt, la ruta principal deixa de ser només web i passa a ser:

- **troballa dominant:** credencial vàlida
- **nova branca principal:** `CREDENCIALS`
- **següent pas únic:** provar reutilització per `SSH`
...
Accés correcte a la màquina com a `nathan`:
...
- usuari efectiu: `nathan`
- UID/GID: `1001`
- `user.txt` visible al home de l’usuari

---

## 7. Obtenció de `user.txt`

### Lectura de la flag d’usuari
...
### Ajust de terminal per treballar amb més comoditat

```bash
export TERM=xterm
```

Aquest ajust no forma part de l’explotació, però sí del treball operatiu durant la sessió SSH.
...
**Resultat observat:** apareixen els binaris SUID típics del sistema, juntament amb entrades sota `snap`, però en el material conservat no hi ha cap via d’explotació SUID validada a partir d’aquesta cerca.

### Cerca de capabilities
...
### Verificació específica del binari interessant
...
**Interpretació útil:** `/usr/bin/python3.8` pot invocar `setuid(0)` gràcies a la seva capability, cosa que obre una via directa d’escalada.

---

## 9. Troballa clau: capabilities sobre Python

Abans d’executar l’escalada, es confirma el context actual:
...
A continuació s’utilitza Python per elevar l’UID efectiu a `0` i llançar una shell:
...
La shell obtinguda eleva correctament a `uid=0`, tot i que conserva `gid=1001(nathan)` a la sortida mostrada.
Per a aquest laboratori això és suficient, ja que permet llegir la flag de root i operar com a superusuari.
...
**Observació important:** al principi `cat root.txt` falla perquè la shell continua situada al home de `nathan`, no a `/root`.

La cerca confirma la ubicació correcta:
...
Amb això, la màquina **CAP** queda resolta.
...
1. Preparació de l’entorn i connexió a HTB
2. Reconeixement inicial
3. Descobriment de tres serveis: `FTP`, `SSH`, `HTTP`
4. Identificació de la web com a superfície dominant inicial (`Security Dashboard`)
5. Accés a `Security Snapshot (5 Second PCAP + Analysis)`
6. Descobriment de la captura útil a `0.pcap`
7. Anàlisi de `0.pcap` amb Wireshark
8. Extracció de credencials FTP en text clar
9. Reutilització de `nathan / Buck3tH4TF0RM3!` per SSH
10. Obtenció de `user.txt`
11. Enumeració local
12. Troballa de `cap_setuid` sobre `/usr/bin/python3.8`
13. Escalada a root mitjançant `os.setuid(0)`
...
CAP és un laboratori curt però molt didàctic perquè encadena tres idees netes:

- una exposició web que permet arribar a captures PCAP;
- una credencial reutilitzable visible en trànsit de xarxa sense xifrar;
- una escalada local simple basada en **Linux capabilities**.

## 13. Annex A — Notes originals íntegres

> A partir d’aquest punt es conserva el material original complet, sense eliminar contingut.
