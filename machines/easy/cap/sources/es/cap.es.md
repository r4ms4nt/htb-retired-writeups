# HTB CAP — Explotación completa ordenada

> Documento maestro reconstruido y ordenado a partir de las notas originales del caso.
>
> **Criterio aplicado:**
...
Para dejar este laboratorio claro de cara al futuro, conviene fijar tres puntos:

- **Vector inicial validado en este laboratorio:** acceso no autenticado al dashboard web y descarga de capturas PCAP mediante ruta numérica (`/data/<id>`), con una captura útil en `0.pcap`.
- **Pivot real validado:** extracción de credenciales FTP en texto claro desde `0.pcap` y reutilización exitosa en **SSH** para el usuario `nathan`.
- **Privesc validada en este laboratorio:** abuso de **Linux capabilities** sobre `/usr/bin/python3.8`, concretamente `cap_setuid`, para elevar a `uid=0`.

### Nota editorial adicional

En las notas originales aparece esta frase:

> “Primero, revisamos el contenido de la carpeta home de nathan y vemos que hay un archivo llamado notes.txt.”

Sin embargo, en la salida de `ls -la` conservada en el propio material no aparece `notes.txt`.
Para no perder el contexto histórico, la frase original se conserva en el anexo, pero en el documento ordenado no se toma como hecho verificado.
...
3. Enumeración web y cierre de fase 1
4. Descubrimiento del recurso PCAP útil
5. Análisis de `0.pcap` y extracción de credenciales
6. Acceso SSH como `nathan`
7. Obtención de `user.txt`
8. Enumeración local para privesc
9. Hallazgo clave: capabilities sobre Python
10. Escalada a root
11. Obtención de `root.txt`
12. Resumen técnico final
13. Anexo A — Notas originales íntegras
...
**Objetivo de este paso:** conectarse a la VPN de Hack The Box para poder interactuar con la máquina objetivo.

### Fijar la IP objetivo

```bash
settarget 10.129.28.170 CAP
```

**Objetivo de este paso:** dejar visible en el entorno de trabajo la IP y el nombre de la máquina objetivo.
...
- `80/tcp` → `gunicorn`
- título web observado: **Security Dashboard**

**Interpretación útil:** la superficie dominante del caso pasa a ser la **web**, porque no solo hay un puerto 80 abierto, sino una aplicación con nombre propio y backend identificable.

---

## 3. Enumeración web y cierre de fase 1

Tras acceder a `http://10.129.28.170/`, la aplicación muestra un **dashboard de seguridad** y se observa una sesión o contexto visual asociado a **Nathan**.

En ese momento, el cierre razonable de fase 1 queda así:

- **superficie dominante:** web
- **rama principal:** `WEB-BASE`
- **ramas secundarias:** FTP, SSH
- **siguiente paso único:** perfilar la funcionalidad real del dashboard y revisar los recursos expuestos por la aplicación

Esta decisión se justifica porque:

- la web devuelve una aplicación real, no una landing vacía;
- FTP y SSH aún no ofrecen acceso útil por sí mismos;
- todavía no se había confirmado credencial válida ni login reutilizable.

---

## 4. Descubrimiento del recurso PCAP útil

Dentro del dashboard se accede a la funcionalidad:

```text
Security Snapshot (5 Second PCAP + Analysis)
```

La navegación lleva a una ruta numerada:
...
La URL usa un identificador numérico, y además aparece un botón de descarga de captura.
Esto sugiere que la aplicación permite acceder a diferentes PCAP en función del ID.

### Prueba realizada

- descarga de `1.pcap`
- cambio manual de la URL hacia `http://10.129.28.170/data/0`
- descarga de `0.pcap`

### Resultado observado

- `1.pcap` está vacío
- `0.pcap` contiene tráfico útil

**Conclusión:** el ID numérico es relevante y la captura útil para el caso es `0.pcap`.

---

## 5. Análisis de `0.pcap` y extracción de credenciales

El archivo `0.pcap` se abre con **Wireshark**.

### Hallazgo clave

Dentro de la captura se observa una sesión **FTP en texto claro** con estas credenciales:
...
Por tanto, la credencial queda validada **al menos para FTP**.

### Otros comandos visibles en la captura
...
La operación `RETR notes.txt` termina en error:

```text
550 Failed to open file.
```

Por tanto, el valor real del PCAP no está en ese archivo, sino en la **credencial reutilizable** expuesta en claro.

### Cambio correcto de rama

En este punto la ruta principal deja de ser solo web y pasa a ser:

- **hallazgo dominante:** credencial válida
- **nueva rama principal:** `CREDENCIALES`
- **siguiente paso único:** probar reutilización en `SSH`
...
Acceso correcto a la máquina como `nathan`:
...
- usuario efectivo: `nathan`
- UID/GID: `1001`
- `user.txt` visible en el home del usuario
...
### Ajuste de terminal para trabajar con más comodidad

```bash
export TERM=xterm
```

Este ajuste no forma parte de la explotación, pero sí del trabajo operativo durante la sesión SSH.
...
**Resultado observado:** aparecen los binarios SUID típicos del sistema, junto a entradas bajo `snap`, pero en lo conservado no hay una vía de explotación SUID validada a partir de esta búsqueda.
...
### Verificación específica del binario interesante
...
**Interpretación útil:** `/usr/bin/python3.8` puede invocar `setuid(0)` gracias a su capability, lo que abre una vía directa de escalada.

---

## 9. Hallazgo clave: capabilities sobre Python

Antes de ejecutar la escalada, se confirma el contexto actual:
...
A continuación se usa Python para elevar el UID efectivo a `0` y lanzar una shell:
...
### Confirmación del nuevo contexto
...
La shell conseguida eleva correctamente a `uid=0`, aunque conserva `gid=1001(nathan)` en la salida mostrada.
Para este laboratorio eso es suficiente, ya que permite leer la flag de root y operar como superusuario.
...
**Observación importante:** al principio `cat root.txt` falla porque la shell sigue situada en el home de `nathan`, no en `/root`.

La búsqueda confirma la ubicación correcta:
...
Con esto, la máquina **CAP** queda resuelta.
...
1. Preparación del entorno y conexión a HTB
2. Reconocimiento inicial
3. Descubrimiento de tres servicios: `FTP`, `SSH`, `HTTP`
4. Identificación de la web como superficie dominante inicial (`Security Dashboard`)
5. Acceso a `Security Snapshot (5 Second PCAP + Analysis)`
6. Descubrimiento de captura útil en `0.pcap`
7. Análisis de `0.pcap` con Wireshark
8. Extracción de credenciales FTP en texto claro
9. Reutilización de `nathan / Buck3tH4TF0RM3!` en SSH
10. Obtención de `user.txt`
11. Enumeración local
12. Hallazgo de `cap_setuid` sobre `/usr/bin/python3.8`
13. Escalada a root mediante `os.setuid(0)`
...
### Idea fuerza del caso

CAP es un laboratorio corto pero muy didáctico porque encadena tres ideas limpias:

- una exposición web que permite llegar a capturas PCAP;
- una credencial reutilizable visible en tráfico de red sin cifrar;
