# HTB Return — Final didactic Markdown

## 1. Case introduction

Return is an easy-difficulty Windows machine whose compromise chain relies on an insecure configuration in a printer administration panel. The entry point was not a version-based exploit or a complex Active Directory vulnerability, but an operational misconfiguration: the panel allowed the configured LDAP server to be modified and, when doing so, the backend attempted to authenticate against the new destination using a service account.

The full resolution was structured through this chain:

```text
Printer administration panel on IIS
→ visible LDAP configuration
→ LDAP server changed to the attacker IP
→ svc-printer credentials captured
→ WinRM validation
→ shell as return\svc-printer
→ membership in Server Operators
→ modification of the vss service binPath
→ execution as LocalSystem
→ x64 Meterpreter session stabilized through migration
→ NT AUTHORITY\SYSTEM
→ root.txt read
→ vss service restored
```

The machine teaches three especially important ideas:

1. An apparently simple panel can contain domain-integration settings that are worth more than generic web enumeration.
2. In Windows, the compromised user’s groups are decisive. In this case, `Server Operators` was the finding that turned a service account into a privilege-escalation path.
3. Service-based exploitation requires traceability: document the original service, modify it, execute it, confirm the context, and restore it.

---

## 2. Laboratory preparation and startup

### Automated preparation

The machine was started with the custom `Inici-HTB` script, used to organize the working environment, validate connectivity, and generate initial evidence:

```bash
Inici-HTB Return 10.129.95.241 easy
```

This step is not part of the exploitation. Its purpose is to prepare the case for methodical work:

- create the directory structure;
- check connectivity;
- estimate the operating system;
- launch a full port scan;
- extract open ports;
- run service enumeration;
- generate an initial note with the suggested dominant surface;
- leave a single next step.

The expected case structure was oriented toward separating evidence and later actions:
...
The didactic reading of this phase is simple: before looking for a vulnerability, it is necessary to know what surface actually exists. This way of working avoids opening branches by intuition and forces decisions based on evidence.

### Connectivity and initial estimation

Connectivity was correct:

```text
64 bytes from 10.129.95.241: icmp_seq=1 ttl=127 time=48.3 ms
```

The TTL-based estimate suggested Windows:

```text
10.129.95.241 (ttl -> 127): Windows
```

This signal should not be treated as an absolute truth, but as a reasonable clue. In this case, it was consistent with the rest of the later enumeration.
...
The full scan detected an extensive surface clearly associated with Windows/Active Directory:
...
| Port | Service | Technical reading |
|---:|---|---|
| 53 | DNS | Common signal in a domain controller or domain environment |
| 80 | HTTP / IIS | Visible web panel |
| 88 | Kerberos | Domain authentication |
| 389 / 636 | LDAP / LDAPS | Active Directory directory services |
| 445 | SMB | Shares / domain |
| 5985 | WinRM | Remote PowerShell access if credentials are available |
| 9389 | ADWS | Active Directory Web Services |
| 47001 | HTTPAPI / auxiliary WinRM | Auxiliary Microsoft service, not the business web application |

The surface could have suggested a pure AD branch, but port `80` returned a very specific title:

```text
http-title: HTB Printer Admin Panel
```

This changed the work priority. It was not a generic website, but an application with a possible direct relationship to the domain infrastructure.
...
The host was also identified as `PRINTER`:
...
A time skew was also observed:

```text
clock-skew: 18m33s
```

In an advanced Kerberos chain, the `clock-skew` could be critical. In this case it was not the main path, but it was correctly recorded as contextual data.

---

## 4. Dominant surface decision

### Verified facts

The machine exposed domain services, but also an HTTP panel with a specific name:
...
```text
Dominant surface:
Windows / Active Directory with an IIS printer web panel on port 80.

Main branch:
WEB-BASE oriented toward the printer administration panel.

Secondary branches:
AD / SMB / LDAP / Kerberos.
WinRM pending credentials.
SMB pending anonymous-access or domain-information checks.
```

### Didactic explanation

On a host with Kerberos, LDAP, SMB, and WinRM, the Active Directory branch must be kept in mind. However, the printer panel was more actionable at that moment because it offered a concrete, browsable surface potentially connected to LDAP.

Corporate printer panels often store settings such as:

- LDAP server;
- SMB server;
- domain;
- service user;
- password or integration secret;
- scan or storage paths.

Therefore, before forcing SMB, Kerberos, or WinRM, it made sense to review the web panel and look for reusable configuration.

---

## 5. Initial SMB enumeration

`enum4linux` was executed to check whether SMB provided useful information without credentials:
...
However, the useful queries failed due to permissions:

```text
NT_STATUS_ACCESS_DENIED
```

The following were not obtained:

- users;
- useful shares;
- password policy;
- RID cycling;
- printer information through spoolss.

### Interpretation

SMB was useful to confirm domain context, but it did not open an operational path without credentials. It therefore remained a secondary branch.

This phase illustrates an important distinction: a tool confirming a partial session or minimal information does not mean that branch is exploitable. Here SMB provided context, not access.

---

## 6. Review of the web panel

### Download and initial reading

The main page was downloaded:

```bash
curl -sS -D content/http_root_headers.txt http://10.129.95.241/ -o content/http_root.html
```

Useful references were then filtered:

```bash
grep -Ei 'href|settings|printer|ldap|server|username|password|domain|port' content/http_root.html
```

The output confirmed the panel title and showed internal links:
...
### Initial error with the `/settings` route
...
This result did not discard the configuration section. The HTML had shown the exact route `settings.php`, not `/settings`.

### Lesson from this point

A route error must not be turned into branch dismissal. The real link was in the HTML. The correct validation was to follow the observed route:
...
## 7. LDAP configuration extraction from `settings.php`

### Requesting the correct route

The configuration page was downloaded:

```bash
curl -sS -D content/http_settings_php_headers.txt http://10.129.95.241/settings.php -o content/http_settings_php.html
```

Relevant fields were filtered:
...
The form used the `POST` method:

```html
<form action="" method="POST">
```

The truly modifiable field was:

```html
<input type="text" name="ip" value="printer.return.local"/>
```

And the panel showed LDAP configuration:
...
This was the first truly exploitable finding of the case. The panel not only showed a service user, but allowed the LDAP server to be modified through the `ip` parameter.

That opened a very concrete hypothesis:

```text
If the backend tries to validate or save the new configuration,
it may connect to the indicated LDAP server using svc-printer.
```

The next step was not to guess the password, but to observe whether the target itself sent it when configuring a controlled LDAP server.

---

## 8. LDAP credential capture

### Preparing the attacker IP

First, the VPN interface IP was obtained:
...
### Controlled LDAP listener

Because the panel indicated LDAP port `389`, a listener was started on that port:

```bash
sudo nc -lvnp 389 | tee content/ldap_capture_svc_printer.txt
```

### Sending the form with a controlled LDAP server

The form submission was reproduced by modifying only the real observed parameter:
...
The identity appeared inside the flow:
...
The application attempted to authenticate against the configured LDAP server and sent the credential for `svc-printer`.

The finding changed the main branch:

```text
Exposed panel configuration
→ domain credential recovered
→ validation against remote services
```

System access was not assumed yet. The credential had to be validated against existing services. WinRM was the strongest candidate because it was already open on `5985`.

---

## 9. Credential validation through WinRM

### Quick check with NetExec

The credential was validated against WinRM:
...
A session was opened with Evil-WinRM:

```bash
evil-winrm -i 10.129.95.241 -u 'svc-printer' -p '1edFg43012!!'
```

The effective identity was confirmed:
...
### Context error: Linux commands in PowerShell

During the session, commands such as `id` and `ls -la` were executed even though they do not belong to PowerShell:
...
This did not affect exploitation, but it left a useful operational lesson. When entering through WinRM, the operator is working in PowerShell, not in a Linux shell.
...
| `cat file` | `type file` or `Get-Content file` |

---

## 10. Initial local enumeration and user flag

### Identity, SID, and groups

From Evil-WinRM, the context was documented:
...
`Remote Management Users` explains the WinRM access. Without that permission, a valid domain credential would not necessarily have allowed a remote shell.

`Server Operators` was the key privilege-escalation finding. This group can operate certain system services. If a user can modify the execution path of a service running as `LocalSystem`, that ability can be turned into privileged execution.

`SeBackupPrivilege` and `SeRestorePrivilege` were also interesting, but they remained secondary branches because `Server Operators` offered a more direct path coherent with the case.

### First flag

The user desktop was reviewed:
...
## 11. Escalation analysis: Server Operators and the `vss` service

### Why `vss` was reviewed

The escalation hypothesis was:

```text
svc-printer belongs to Server Operators
→ can operate services
→ if a service runs as LocalSystem and can be modified
→ a controlled binary can be executed as SYSTEM
```

The `vss` service was chosen first because it is a known Windows service, on demand, and in this machine it allowed querying and manipulation.

### Initial service tests

A global Service Control Manager query failed:
...
However, the specific `vss` query worked:
...
The WMI query using `Get-CimInstance` failed due to permissions:

```text
Access denied
```

But `sc.exe` provided the necessary information.
...
### Confirmation of `binPath` modification

The critical test was to modify `BINARY_PATH_NAME` while keeping the original value:
...
This result validated that `svc-printer` could modify the service configuration.

The combination was enough to justify the path:

```text
modifiable service
+ service executed as LocalSystem
...
## 12. File-based validation attempts and issues encountered

### Initial `whoami` test

A simple command was attempted through the service:
...
This error was expected: `cmd.exe /c whoami` is not a real Windows service. The Service Control Manager expects the process to behave as a service; a one-shot command exits quickly or does not respond to the expected protocol.
...
### Quoting and redirection issues

The syntax was improved with quotes and error redirection:

```powershell
sc.exe config vss binPath= 'C:\Windows\System32\cmd.exe /c "whoami > C:\Windows\Temp\whoami_system.txt 2>&1"'
```

But `sc.exe` returned its usage help, indicating that the command had not been applied:
...
The lesson here was important: when `sc.exe` displays usage help, nothing should be assumed to have been applied. It must always be confirmed with:
...
To prevent PowerShell from interpreting redirections and quotes, the stop-parsing operator was used:
...
The service again returned `1053`, but the file existed. The issue was that the user could not read it:
...
These attempts did not invalidate the technique. On the contrary, they confirmed that:

- the `binPath` could be changed;
- the service attempted to execute the configured value;
- error `1053` was not a discard criterion;
- file redirection added too much friction due to syntax and permissions.

The correct decision was to abandon file-based validation and move to a reverse connection.

---

## 13. First tactical change: `nc.exe` as a reverse shell

### Binary preparation

Initially, `nc.exe` was uploaded from an absolute path:

```powershell
upload /usr/share/windows-resources/binaries/nc.exe
```

Evil-WinRM interpreted it as a path relative to the local laboratory directory:
...
`upload nc.exe` was also attempted, but the file did not yet exist in the local directory:

```text
No such file or directory /home/r4mon/pentest/targets/HTB/easy/Return/nc.exe
```

### Context error: searching in Parrot from Evil-WinRM

A Linux command was accidentally executed inside remote PowerShell:

```powershell
find /usr/share -type f -iname 'nc.exe' 2>/dev/null
```

PowerShell interpreted `/dev/null` as a Windows path:

```text
Could not find a part of the path 'C:\dev\null'
```

This was a useful operational error to document. The contexts had to be clearly separated:

```text
Local Parrot terminal ≠ remote PowerShell session through Evil-WinRM
```

### Correct location of `nc.exe`

The binary was searched from the local Parrot terminal:
...
It was copied to the laboratory directory:
...
### Listener and `vss` modification

A listener was opened in Parrot:
...
`vss` was configured to execute `nc.exe`:
...
When the service was started, a connection arrived:
...
The shell dropped before `whoami` could be confirmed.

This was enough to confirm that the service executed the binary, but not a stable session. The service was restored:
...
## 14. Second tactic: Meterpreter x86

### Payload generation

`msfvenom` availability was checked:
...
A Windows x86 payload was generated:
...
Metasploit was opened and the handler loaded:

```text
use exploit/multi/handler
```

Metasploit initially selected a generic payload:

```text
generic/shell_reverse_tcp
```

It was corrected to match the binary:
...
It was confirmed on the victim:
...
When the service was started, Metasploit received a session:
...
Execution was confirmed, but stability was still insufficient. The next hypothesis was that the x86 payload was not the best fit for a 64-bit Windows Server 2019 machine.

`vss` was restored again.

---

## 15. Third tactic: x64 Meterpreter with automatic migration

### x64 payload generation

An x64 executable was generated:
...
The session opened, but another context error occurred: `whoami` was attempted inside Meterpreter.

```text
[-] Unknown command: whoami. Run the help command for more details.
```

The correct command in Meterpreter was:

```text
getuid
```

The session died before this could be confirmed.

### Automatic migration

To prevent the session from dying before manual migration, automatic migration was configured in Metasploit:
...
`vss` was configured again with `shell64.exe` and the service was started.

Metasploit received the session and migrated automatically:
...
In Meterpreter, the correct command was used:
...
Privilege escalation was confirmed.

---

## 16. Root flag and cleanup
...
### Final service restoration

After obtaining the final flag, `vss` was restored:
...
This restoration is an important part of traceability. The exploitation temporarily modified a sensitive system configuration; the proper closure requires demonstrating that the service returned to its original state.
...
2. Dominant surface:
   Printer web panel on IIS.

3. Web finding:
   settings.php exposes LDAP configuration.

4. Initial primitive:
   The Server Address field can be modified through POST using the ip parameter.

5. Capture:
   By pointing the LDAP server to the attacker IP, the target sends credentials.

6. Recovered credential:
   return\svc-printer : 1edFg43012!!

7. Validation:
   WinRM accepts the credential.

8. Access:
   Evil-WinRM as return\svc-printer.

9. Local finding:
   svc-printer belongs to Server Operators.

10. Escalation:
    vss runs as LocalSystem and allows binPath modification.

11. Execution:
    shell64.exe executed through vss.

12. Stabilization:
    x64 Meterpreter + AutoRunScript migrate.

13. Privileges:
    NT AUTHORITY\SYSTEM.

14. Closure:
    root.txt read and vss restored.
```
...
### 18.1. A simple panel can be more important than broad AD enumeration

Although the machine exposed many domain services, the real path began in the web panel. The key was not to treat HTTP as secondary just because Kerberos or LDAP existed.
...
### 18.2. Printer panels often integrate with LDAP or SMB

A corporate multifunction printer may need to:

- query users in LDAP;
- save scans to shares;
- authenticate against the domain;
- use service accounts.

For that reason, a `Settings` section with `Server Address`, `Server Port`, `Username`, and `Password` must be treated as a sensitive finding.

### 18.3. A found credential is not the same as final access

The `svc-printer` password became real access only after it was validated against WinRM:
...
observed credential
→ valid credential
→ credential exploitable through WinRM
```

### 18.4. In Windows, groups matter as much as the user

`svc-printer` was not a direct administrator, but it belonged to `Server Operators`. That membership allowed service manipulation.
...
The group output was more important than the contents of the user directory.

### 18.5. `sc.exe qc` must accompany `sc.exe config`

Every service change was validated with:

```powershell
sc.exe qc vss
```

This avoided assuming that a modification had worked when `sc.exe` had only displayed usage help.

Recommended pattern:

```text
sc.exe config ...
→ check ChangeServiceConfig SUCCESS
→ sc.exe qc ...
→ confirm BINARY_PATH_NAME
→ start service
```

### 18.6. Error 1053 does not necessarily invalidate execution

When running commands that are not real services, `sc.exe start` may return:

```text
StartService FAILED 1053
```

In this case, the error did not mean execution was impossible. It meant that the launched process did not respond like a Windows service.

The real indicator had to be:
...
### 18.7. Separating local and remote context avoids errors

A mistake occurred when a Linux command was executed inside Evil-WinRM:

```powershell
find /usr/share -type f -iname 'nc.exe' 2>/dev/null
```

PowerShell tried to interpret `/dev/null` as `C:\dev\null`.
...
Evil-WinRM:
  remote PowerShell, sc.exe, Get-ChildItem, upload.

Meterpreter:
  getuid, cat, migrate, ps.
```

### 18.8. Meterpreter requires its own commands

Inside Meterpreter, `whoami` is not the proper command:

```text
[-] Unknown command: whoami
```

To confirm the user, use:

```text
getuid
```

### 18.9. Stability may require migration

The `nc.exe` shell and the first Meterpreter sessions died quickly. The stable session arrived when these were combined:
...
Automatic migration allowed the session to leave the initial `shell64.exe` process and move into a more stable process:

```text
[+] Successfully migrated into process 688
```

### 18.10. Restoring the service is part of the technical closure

After modifying `vss`, the proper closure was to restore:
...
## 19. Discarded or undeveloped points

### SMB without credentials

SMB confirmed the `RETURN` domain, but did not allow useful enumeration without credentials. It remained a secondary branch and did not need to be revisited to complete the machine.

### SeBackupPrivilege / SeRestorePrivilege

The user had interesting privileges such as `SeBackupPrivilege` and `SeRestorePrivilege`, but they were not developed because the `Server Operators` path was direct and sufficient.

### File-based validation

Validation through `whoami` redirection to a file proved noisy due to quoting issues, `1053`, and read-permission problems. It was correctly abandoned in favor of a reverse session.

---

## 20. Editorial criterion applied

A different exploitation path has not been reconstructed. The main body preserves the real laboratory chain, including useful context errors, failed file-based validation attempts, stability issues, and the final restoration of `vss`.

No technical absurdities in the annex have been corrected. The main body normalizes and reorganizes mechanical repetition, rigid blocks, and the operational sequence to produce a clearer technical narrative. The original operational notes are preserved at the end for traceability.

---

# Appendix I — Original operational notes preserved

The following block preserves the original operational notes of the case for documentary traceability. The main body already contains a consolidated, ordered, and didactic version of the resolution.
