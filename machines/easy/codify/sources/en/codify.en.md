...
-> shell as svc
-> SQLite tickets.db
-> bcrypt hash for joshua
-> offline cracking
-> SSH as joshua
-> sudo on mysql-backup.sh
-> bypass with *
-> pspy captures real password
-> su root
```
...
```text
Dominant surface: HTTP / web application
Main branch: WEB-BASE
Secondary branches: SSH
Single next step: resolve codify.htb and observe the web application on 80 and 3000
```
...
```text
80/tcp   -> Apache redirecting to codify.htb
3000/tcp -> Node.js Express application titled Codify
```
...
-> vm2 sandbox
-> observed version 3.9.16
-> POST /run endpoint
-> code sent in Base64 inside JSON
```
...
```text
Can child_process be imported directly?
```

The question becomes:

```text
Can the vm2 sandbox be escaped despite these restrictions?
```
...
It is important to distinguish the two prompts:
...
Then the script prompt appears:
...
2. WEB-BASE:
   - Apache on 80.
   - Express on 3000.
   - Same application through hostname and direct port.
   - Public routes: /about, /editor, /limitations.

3. Technology:
   - Application for testing Node.js code.
   - Sandbox identified: vm2.
   - Observed version: 3.9.16.
   - Real endpoint: POST /run.
   - Code sent in Base64.

4. Validations:
   - /run executes benign code.
   - child_process is blocked by direct import.
   - The correct path is sandbox escape, not direct require.

5. Foothold:
   - PoC compatible with vm2 3.9.16.
   - id executed as svc.
   - Reverse shell as svc.

6. Lateral movement:
   - /var/www/contact/tickets.db located.
   - Valid SQLite database.
   - users table contains bcrypt hash for joshua.
   - Hashcat recovers spongebob1.
   - SSH as joshua.
   - user.txt obtained.

7. Escalation:
   - sudo -l allows /opt/scripts/mysql-backup.sh as root.
   - Script reads /root/.creds.
   - Unsafe comparison allows bypass with *.
   - mysql/mysqldump expose password through arguments.
   - pspy captures kljh12k3jhaskjh12kjh3.
   - su root works.
   - root.txt obtained.
```
...
```text
A filter on dangerous modules is not the same as a robust sandbox.
```
...
```text
If an exploit fails, first make sure the basic flow works.
```
...
```text
Extract artifact -> classify hash -> offline cracking -> validate reuse
```
...
### 16.9 Distinguishing prompts avoids errors

During escalation there were two different prompts:
...
## 17. Editorial corrections applied

Editorial and clarity corrections were applied to the main body without altering the technical chain:

- `Hack Yhe Box` was corrected to `Hack The Box`;
- the real path was normalized as `/var/www/contact`, explaining that `/var/www/contacts` did not exist in the instance;
- the incorrect expression `Análisis observada` was changed to `Observed response`;
- the repetitive rigidity of “Objective / Verified facts / Method” blocks in every micro-phase was removed, integrating their content into a natural technical narrative;
- relevant operational incidents were kept as lessons, but separated from machine findings;
- no new steps were invented and the real exploitation was not replaced with an alternative chain.

The original notes are preserved in full below as a traceability appendix.


# Appendix A — Original notes preserved

From this point onward, the original Spanish working material is preserved to maintain documentary traceability of the case.
