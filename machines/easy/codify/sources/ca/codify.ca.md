...
## Informe tècnic didàctic

**Màquina:** Codify
**Plataforma:** Hack The Box
**Sistema:** Linux
**Dificultat:** Easy
**Data de treball:** 2026-05-06, zona Europe/Madrid
**Estat:** Resolta
**Usuari inicial obtingut:** `svc`
**Usuari lateral:** `joshua`
**Usuari final:** `root`
...
Codify exposa una aplicació web desenvolupada amb Node.js/Express que permet executar fragments de codi JavaScript dins d’un entorn de sandbox. La mateixa aplicació documenta l’ús de `vm2` i enllaça la versió `3.9.16`, fet que converteix la superfície web en una via candidata per a un escape de sandbox.

La resolució es basa en una cadena d’evidències progressiva:

1. enumeració inicial amb `Inici-HTB`;
2. identificació d’una aplicació Node.js/Express als ports `80` i `3000`;
3. detecció de `vm2 3.9.16` com a tecnologia de sandbox;
4. validació benigna de l’endpoint `/run`;
5. comprovació del bloqueig directe de mòduls sensibles com `child_process`;
6. explotació d’una vulnerabilitat pública compatible amb la versió observada;
7. obtenció de shell com `svc`;
8. cerca d’artefactes locals de l’aplicació;
9. extracció d’un hash bcrypt de `joshua` des d’una base SQLite;
10. craqueig offline del hash i accés SSH com `joshua`;
11. revisió de permisos `sudo`;
12. anàlisi d’un script de còpia de seguretat executable com `root`;
13. abús d’una comparació insegura en Bash;
14. captura de la contrasenya real observant processos amb `pspy`;
15. autenticació com `root` i lectura de la flag final.

La cadena completa queda resumida així:
...
-> bypass amb *
-> pspy captura la contrasenya real
-> su root
```
...
El cas va començar amb l’execució de l’script propi `Inici-HTB`, utilitzat per preparar l’entorn de treball, comprovar connectivitat i generar una base inicial d’evidències.

```bash
Inici-HTB Codify 10.129.40.127 easy
```

Aquest script no explota res. El seu valor és ordenar el laboratori des de l’inici i deixar una estructura mínima de treball:
...
La idea didàctica d’aquesta fase és important: abans de buscar vulnerabilitats, cal saber quina superfície real existeix. En una màquina Linux senzilla, una arrencada deficient sol provocar dos errors: saltar a l’explotació sense evidència o perdre temps en branques secundàries.

### 2.2 Evidència inicial

La màquina va respondre al ping:

```text
64 bytes from 10.129.40.127: icmp_seq=1 ttl=63 time=47.4 ms
```

El TTL `63` i la sortida de `whichSystem.py` apuntaven a Linux:

```text
10.129.40.127 (ttl -> 63): Linux
```

L’escaneig complet de ports va detectar tres ports oberts:
...
L’escaneig de serveis va precisar millor la superfície:
...
A més, el port `80` mostrava una redirecció cap a:
...
La primera decisió metodològica va ser clara:

```text
Superfície dominant: HTTP / aplicació web
Branca principal: WEB-BASE
Branques secundàries: SSH
Pas següent únic: resoldre codify.htb i observar l’aplicació web pels ports 80 i 3000
```

SSH va quedar anotat com a via secundària perquè estava obert, però encara no hi havia credencials. La superfície dominant era web, amb dues senyals molt fortes:

```text
80/tcp   -> Apache amb redirecció a codify.htb
3000/tcp -> aplicació Node.js Express amb títol Codify
```

---

## 3. Normalització del hostname i comparació de superfícies HTTP

### 3.1 Resolució de `codify.htb`

Nmap ja havia revelat el hostname `codify.htb`, de manera que es va afegir al fitxer `/etc/hosts`:
...
Aquest pas era necessari perquè el port `80` redirigia cap al nom de host. Si la màquina atacant no resolia `codify.htb`, les peticions HTTP no representarien correctament el comportament real de l’aplicació.

### 3.2 Comparació entre IP, hostname i port 3000

Es van revisar capçaleres HTTP per IP, per hostname i pel port directe `3000`:
...
El hostname retornava `200 OK` i mostrava Express darrere d’Apache:
...
El port `3000` retornava la mateixa longitud i també `X-Powered-By: Express`:
...
La interpretació va ser que Apache al port `80` actuava com a frontal o proxy, mentre que el port `3000` exposava directament l’aplicació Express. En tots dos casos s’arribava al mateix backend.

### 3.3 Extracció de rutes públiques

Es van extreure recursos visibles de la pàgina principal:
...
`robots.txt` no existia com a recurs vàlid:

```text
Cannot GET /robots.txt
```

`whatweb` va confirmar la lectura tecnològica:
...
### 3.4 Tancament de WEB-BASE inicial

L’aplicació no mostrava cap CMS, ni cap login, ni cap panell autenticat. Tampoc apareixien descàrregues ni artefactes públics. La senyal rellevant era una altra: la mateixa aplicació es presentava com una plataforma per executar codi Node.js dins d’un sandbox.

El text visible indicava:

```text
Codify uses sandboxing technology to run your code.
```

Això va canviar el focus. La web deixava de ser una superfície genèrica i passava a ser una aplicació que executa codi no confiable.

---

## 4. Identificació de la tecnologia de sandbox

### 4.1 Descàrrega de pàgines informatives

Es van guardar les rutes públiques rellevants:
...
La raó de guardar aquestes pàgines era conservar evidència local i poder cercar termes tècnics sense dependre del navegador.

### 4.2 Cerca de termes clau

Es van cercar indicadors relacionats amb execució de codi, sandboxing i mòduls de Node.js:

```bash
grep -RniE 'sandbox|node|vm2|module|require|child_process|limitations|restricted|editor|code' content/web/
```

La pàgina `/about` identificava la tecnologia de sandbox:
...
Aquesta va ser una senyal de versió molt forta, encara que cal descriure-la amb precisió: és una versió observada des de l’aplicació, no una versió confirmada des del sistema de fitxers.

### 4.3 Restriccions declarades per l’aplicació

La pàgina `/limitations` indicava que alguns mòduls estaven restringits:
...
També llistava mòduls permesos:
...
Aquesta informació ajuda a entendre el model defensiu de l’aplicació. No es tracta d’una execució Node.js sense controls; hi ha un intent explícit de bloquejar mòduls perillosos. Tanmateix, aquesta capa no demostra que el sandbox sigui segur.

### 4.4 Flux real de l’editor

El fitxer `editor.html` va revelar com s’envia el codi al backend:
...
El navegador codifica el contingut amb `btoa()`:
...
La superfície tècnica dominant passa a ser:

```text
Aplicació Node.js/Express
-> editor de codi JavaScript
-> sandbox vm2
-> versió observada 3.9.16
-> endpoint POST /run
-> codi enviat en Base64 dins de JSON
```

---

## 5. Validació benigna de l’endpoint `/run`

### 5.1 Per què validar abans d’explotar

Abans d’utilitzar una PoC pública, es va validar el flux de manera benigna. Aquest pas evita confondre un problema de format, Base64, JSON o endpoint amb un error d’explotació.

Es va preparar un codi innocu:

```javascript
console.log("HTB-Codify-test")
```

i es va codificar en Base64:
...
### 5.2 Prova contra `/run`

Es va reproduir des de terminal el que feia el navegador:
...
La mateixa prova va funcionar contra el port directe `3000`:
...
La lectura és clara: l’endpoint `/run` accepta JSON, espera `code` en Base64, executa el JavaScript dins del backend i retorna la sortida al camp `output`.

### 5.3 Validació de mòdul restringit

Després es va comprovar si la restricció sobre `child_process` era real. Es va enviar l’equivalent a:
...
Aquest resultat no descarta la via d’explotació. Al contrari, aclareix el model defensiu: l’aplicació bloqueja importacions directes de mòduls perillosos i delega la seguretat real de l’aïllament en `vm2`.

La pregunta tècnica ja no és:

```text
es pot importar child_process directament?
```

La pregunta passa a ser:

```text
es pot escapar del sandbox vm2 malgrat aquestes restriccions?
```

---

## 6. Candidata pública: escape de sandbox a `vm2`

La revisió de vulnerabilitats públiques associades a `vm2 3.9.16` condueix a **CVE-2023-30547**, una vulnerabilitat d’escape de sandbox a `vm2`. Segons les notes de treball, aquesta candidata afecta versions fins a `3.9.16`, està relacionada amb una sanitització incorrecta d’excepcions i va ser corregida posteriorment.

En aquesta fase se separen tres nivells:

### Fet verificat

L’aplicació utilitza `vm2` i enllaça a `3.9.16`.

### Inferència raonable

La versió en ús pot ser vulnerable a CVE-2023-30547.

### Pendent fins a validació

La vulnerabilitat s’ha de provar a l’endpoint real `/run` i demostrar execució fora del sandbox.

---

## 7. Validació de l’escape de sandbox

### 7.1 Prova mínima amb `id`

La validació inicial es va fer des de la pàgina:

```text
http://codify.htb/editor
```

Es va enganxar una PoC pública adaptada per executar `id`:
...
La importància d’aquesta prova és que `child_process` estava bloquejat per importació directa. Obtenir la sortida d’`id` demostra que l’execució ja no queda limitada al flux normal del sandbox i que s’ha assolit el context del procés Node.js del backend.

### 7.2 Preparació de la reverse shell

Es va crear un script local `rev.sh` a la màquina atacant:
...
Es va iniciar un servidor HTTP per servir l’script:

```bash
python3 -m http.server 8081
```

I un listener per rebre la connexió:

```bash
nc -lnvp 4444
```

Abans d’executar la PoC va caldre confirmar la IP real de la interfície VPN:
...
Aquest control va evitar barrejar IPs a l’script i al `curl` de la PoC.

### 7.3 PoC per obtenir shell

A `/editor` es va executar la PoC modificada per descarregar i executar `rev.sh`:
...
Al listener es va rebre connexió des de la màquina víctima:
...
Es va estabilitzar mínimament la shell:
...
La fase d’accés inicial queda tancada: l’usuari inicial és `svc`.

---

## 8. Enumeració local després del foothold

### 8.1 Identificació d’usuaris interactius

Es va revisar `/etc/passwd` per detectar usuaris locals amb shell:
...
Aquesta sortida va orientar el moviment lateral: si l’aplicació emmagatzemava credencials o hashes, `joshua` era l’usuari local candidat.

### 8.2 Cerca d’artefactes web

La bitàcola de referència apuntava a `/var/www/contacts`, però aquesta ruta no existia a la instància treballada. En lloc d’insistir sobre una ruta no verificada, es va enumerar `/var/www`:
...
Aquest ajust és metodològicament important: la referència orienta, però l’evidència de la màquina mana.

### 8.3 Confirmació de la base SQLite

Es va revisar el directori real:
...
La màquina tenia `sqlite3` instal·lat, així que no va caldre transferir la base per llegir-la.

---

## 9. Extracció de hash des de `tickets.db`

### 9.1 Enumeració de taules

Es van llistar les taules:
...
La taula `users` va passar a ser prioritària, perquè podia contenir credencials o hashes reutilitzables.

### 9.2 Volcat de la taula `users`

Es va mostrar esquema i contingut:
...
El prefix `$2a$12$` va permetre classificar el hash com a bcrypt.

### 9.3 Craqueig offline

El hash es va guardar a la màquina atacant:
...
Es va llançar Hashcat en mode bcrypt:
...
La contrasenya es va craquejar offline, no a la víctima. Això redueix soroll al sistema objectiu i manté la fase de credencials separada de l’enumeració local.

---

## 10. Moviment lateral per SSH

Des de la fase 1 se sabia que SSH estava obert:

```text
22/tcp open ssh OpenSSH 8.9p1 Ubuntu
```

Es va provar la contrasenya recuperada contra l’usuari local `joshua`:
...
La cadena de moviment lateral queda tancada:
...
## 11. Enumeració sudo com `joshua`

### 11.1 Revisió de permisos sudo

La primera comprovació d’escalada local va ser:
...
La prioritat va canviar: ja no importava la base SQLite ni la web. La via principal era ara un script custom executable com `root`.

### 11.2 Inspecció de l’script abans d’executar-lo

Es van revisar permisos, tipus i contingut:
...
Després utilitza la contrasenya real en comandes MySQL:
...
## 12. Anàlisi de la vulnerabilitat de l’script

L’script conté dues debilitats útils encadenades.

### 12.1 Comparació insegura en Bash
...
En Bash, dins de `[[ ... == ... ]]`, el costat dret de la comparació es pot comportar com un patró. Si s’introdueix `*`, pot coincidir amb qualsevol contrasenya.

Es va validar executant:

```bash
sudo /opt/scripts/mysql-backup.sh
```

Quan l’script va preguntar:
...
El bypass quedava confirmat.

### 12.2 Exposició de contrasenya per arguments de procés

Després del bypass, l’script executa `mysql` i `mysqldump` amb la contrasenya en línia de comandes:

```bash
-p"$DB_PASS"
```

Això fins i tot genera advertiments:
...
L’advertiment és una pista excel·lent: si la contrasenya viatja com a argument del procés, pot observar-se temporalment amb una eina com `pspy`.

---

## 13. Captura de la contrasenya real amb `pspy`

### 13.1 Preparació a la màquina atacant

Es va descarregar `pspy64s`:
...
### 13.2 Transferència a la víctima

Des de la sessió SSH com `joshua`:
...
`pspy` es va deixar corrent en una sessió SSH.

### 13.3 Segona sessió SSH per disparar l’script

Es va obrir una altra sessió SSH com `joshua` i es va executar:

```bash
sudo /opt/scripts/mysql-backup.sh
```

És important distingir els dos prompts:

```text
[sudo] password for joshua:
```

Aquí va la contrasenya de `joshua`:

```text
spongebob1
```

Després apareix el prompt de l’script:
...
### 13.4 Contrasenya capturada

`pspy` va capturar el procés `mysqldump`:

```text
/usr/bin/mysqldump --force -u root -h 0.0.0.0 -P 3306 -pkljh12k3jhaskjh12kjh3 mysql
```

Contrasenya recuperada:

```text
kljh12k3jhaskjh12kjh3
```

La línia confirma tres fets:

1. el procés s’executa com `UID=0`;
2. l’script utilitza la contrasenya real llegida des de `/root/.creds`;
3. aquesta contrasenya apareix exposada als arguments del procés.

---

## 14. Escalada final a `root`

Es va provar la reutilització de la contrasenya amb `su root`:
...
### 15.1 Cadena d’explotació

```text
1. Fase 1:
   - Linux detectat per TTL.
   - Ports oberts: 22, 80, 3000.
   - Web dominant.

2. WEB-BASE:
   - Apache al 80.
   - Express al 3000.
   - Mateixa aplicació per hostname i port directe.
   - Rutes públiques: /about, /editor, /limitations.

3. Tecnologia:
   - Aplicació per provar codi Node.js.
   - Sandbox identificat: vm2.
   - Versió observada: 3.9.16.
   - Endpoint real: POST /run.
   - Codi enviat en Base64.

4. Validacions:
   - /run executa codi benigne.
   - child_process està bloquejat per importació directa.
   - La via correcta és escape de sandbox, no require directe.

5. Foothold:
   - PoC compatible amb vm2 3.9.16.
   - Execució d’id com svc.
   - Reverse shell com svc.

6. Moviment lateral:
   - /var/www/contact/tickets.db localitzat.
   - Base SQLite vàlida.
   - Taula users conté hash bcrypt de joshua.
   - Hashcat recupera spongebob1.
   - SSH com joshua.
   - user.txt obtingut.

7. Escalada:
   - sudo -l permet /opt/scripts/mysql-backup.sh com root.
   - Script llegeix /root/.creds.
   - Comparació insegura permet bypass amb *.
   - mysql/mysqldump exposen password per arguments.
...
### 15.3 Credencials i secrets del cas
...
### 16.1 Llegir l’aplicació abans de buscar exploits

Codify ensenya que una web senzilla pot revelar la seva pròpia via d’atac si se’n llegeixen bé les pàgines. `/about`, `/limitations` i `/editor` van aportar més valor que un fuzzing genèric primerenc.
...
-> tecnologia declarada
-> versió observada
-> endpoint funcional
-> validació benigna
-> anàlisi de candidata pública
```

### 16.2 Distingir bloqueig directe de seguretat real

El bloqueig de `child_process` no feia segura l’aplicació. Només impedia una via directa. La protecció real depenia de `vm2`, i la versió observada era vulnerable.

Lliçó:

```text
Un filtre sobre mòduls perillosos no equival a un sandbox robust.
```

### 16.3 Validar el flux abans d’explotar

Abans de la PoC, es va confirmar que `/run` acceptava JSON, requeria Base64 i retornava sortida a `output`. Aquesta validació va evitar errors d’interpretació.

Lliçó:

```text
Si un exploit falla, primer cal saber si el flux bàsic funciona.
```

### 16.4 L’evidència real corregeix la ruta del writeup

La ruta esperada era `/var/www/contacts`, però la màquina tenia `/var/www/contact`.

Lliçó:

```text
La bitàcola orienta; l’evidència mana.
```

### 16.5 Les bases locals d’aplicació són pivots freqüents

`tickets.db` contenia el hash de `joshua`. Aquest patró és molt habitual després d’un foothold web: l’aplicació acostuma a tenir bases locals, fitxers `.env`, configuracions o artefactes que permeten moure’s a un altre usuari.

### 16.6 El craqueig es fa offline

El hash bcrypt es va extreure i es va craquejar a la màquina atacant. No es va executar craqueig a la víctima.

Lliçó:

```text
Extreure artefacte -> classificar hash -> craqueig offline -> validar reutilització
```

### 16.7 No executar scripts sudo sense llegir-los

`sudo -l` va mostrar un script custom. La via correcta va ser llegir-lo abans d’executar-lo. La fallada era a la seva lògica: lectura de `/root/.creds`, comparació insegura i ús de la contrasenya com a argument.

### 16.8 Els advertiments importen
...
no era soroll. Era una pista directa que la contrasenya podia veure’s en processos.

### 16.9 Diferenciar prompts evita errors

A l’escalada hi havia dos prompts diferents:
...
Confondre’ls produeix errors d’autenticació que no tenen a veure amb la vulnerabilitat.

### 16.10 Incidències operatives que no són troballes de la màquina

Durant el laboratori van aparèixer incidències locals útils per recordar, però no s’han de confondre amb vulnerabilitats de Codify:

- utilitzar `path` com a variable a `zsh` va trencar temporalment la resolució de comandes com `curl`, `grep` i `sort`;
- un `heredoc` amb blocs Markdown interns pot quedar mal copiat;
- el pegat brut a la shell es va corregir amb:

```bash
bind 'set enable-bracketed-paste off'
```

Aquestes incidències són útils per a l’aprenentatge operatiu, però no formen part de l’explotació de la màquina.

---

## 17. Correccions editorials aplicades

Al cos principal es van aplicar correccions editorials i de claredat sense alterar la cadena tècnica:

- es va corregir `Hack Yhe Box` a `Hack The Box`;
- es va normalitzar la ruta real com `/var/www/contact`, explicant que `/var/www/contacts` no existia a la instància;
- es va canviar l’expressió errònia `Análisis observada` per `Resposta observada`;
- es va eliminar la rigidesa repetitiva de blocs tipus “Objectiu / Fets verificats / Mètode” a cada microfase, integrant-ne el contingut en una narrativa tècnica natural;
- es van mantenir les incidències operatives rellevants com a lliçons, però separades de les troballes de la màquina;
- no es van inventar passos nous ni es va substituir l’explotació real per una cadena alternativa.

Les notes originals es conserven completes a continuació com a annex de traçabilitat.


# Annex A — Notes originals conservades

A partir d’aquest punt es conserva el material original en espanyol per mantenir la traçabilitat documental del cas.
