# HTB Forest — Didactic technical writeup
...
| Machine | Forest |
| Difficulty | Easy |
| Identified system | Windows Server 2016 Standard 14393 |
| Dominant role | Active Directory Domain Controller |
| Domain | `htb.local` / `HTB.LOCAL` |
| Hostname | `FOREST` |
| FQDN | `FOREST.htb.local` |
| Initial observed IP | `10.129.34.129` |
| Final IP after reconnection | `10.129.95.210` |
| Compromised user | `svc-alfresco` |
| Recovered credential | `svc-alfresco:s3rvice` |
| Controlled account created | `r4mforest:R4mForest!26` |
| Administrator NTLM hash obtained | `32693b11e6aa90eb43d32c72a07ceea6` |
| Status | Solved |

## Executive summary

Forest is an Active Directory-oriented machine. The initial reconnaissance phase quickly shows a Domain Controller profile: DNS, Kerberos, LDAP, SMB, Global Catalog, ADWS and WinRM are exposed on the same host. Although there are also HTTP services associated with Microsoft HTTPAPI, they do not form a main web application; on this machine they are auxiliary signals of Windows services and remote administration.

The compromise chain follows a classic and highly educational sequence:

```text
LDAP anonymous bind
-> domain object enumeration
-> svc-alfresco identification
-> ASREPRoasting
-> offline cracking of the AS-REP
-> valid credential for svc-alfresco
-> WinRM access
-> Account Operators identification
-> relationship analysis with BloodHound
-> abuse of Exchange Windows Permissions with WriteDacl over the domain
-> granting DCSync permissions to a controlled account
-> extraction of the Administrator hash
-> pass-the-hash
-> administrative access and root.txt
```

The machine teaches three central ideas. First, LDAP enumeration in Active Directory can reveal a full path if interpreted correctly. Second, Kerberos is not only used for authentication; it can also become an attack surface when an account has pre-authentication disabled. Third, privilege escalation in Active Directory does not always go through immediate local privileges: it is often built by chaining groups, ACLs and delegated permissions.

## Scope and documentation criteria

This document reconstructs the real lab resolution from the observed evidence. It does not present an alternative path or an invented exploitation route. When an output was ambiguous, it is marked as such. When a test failed because of sequence or environment, it is preserved as part of the technical learning process, because those errors help explain the methodology better.

The main body keeps the useful didactic part of the notes: why each branch was chosen, which command made sense at each point, which output mattered, and how it changed the next decision. At the end, an annex with the complete original notes is included for traceability.

## 1. Lab preparation and startup

The machine was started using the usual workflow through the `Inici-HTB` script, which prepares the target directory, checks connectivity, performs quick system recognition and launches the initial scans.
...
Initial connectivity was correct:
...
The observed TTL was `127`, compatible with a Windows system. The quick identification tool also classified the target as Windows.

### Didactic reading

TTL should not be treated as absolute proof of the operating system, but it is a useful early signal. In this case, that signal was later reinforced by Nmap and SMB, so it was validated as part of a broader evidence set.

## 2. Initial enumeration with Nmap

The full port scan showed a broad surface that was very characteristic of an Active Directory environment.
...
49664-50033/tcp open dynamic RPC services
```

The service scan identified the domain and host:
...
The combination of `53`, `88`, `389`, `445`, `3268`, `3269` and `9389` is a strong signal of a Domain Controller. Port `5985/tcp` indicates WinRM, which will be useful later if a valid credential is obtained. The HTTP services identified as `Microsoft-HTTPAPI/2.0` return `Not Found`, so they do not justify opening a main web branch.

### Branch decision

The dominant surface becomes:

```text
AD / LDAP / Kerberos / SMB
```

The secondary branches are noted as follows:

- WinRM: pending valid credentials.
- SMB: useful for validation and complementary enumeration.
- HTTPAPI: not considered a main web application.

## 3. Local domain resolution

In Active Directory it is advisable to work with domain names and FQDNs, not only IP addresses. Tools related to LDAP, Kerberos or SMB may depend on correct domain resolution.
...
Initially, the resolution was associated with `10.129.34.129`. Later, after an HTB reconnection, the machine IP changed to `10.129.95.210`, so `/etc/hosts` had to be corrected:
...
The final resolution correctly pointed to:
...
When an HTB machine is reconnected and its IP changes, it is not enough to modify manual commands. In Active Directory, `/etc/hosts` must also be reviewed, because stale resolution can cause confusing Kerberos, LDAP or Impacket errors.

## 4. LDAP anonymous bind

Since Nmap identified LDAP and Global Catalog, the next logical step was to check whether the domain allowed anonymous queries.
...
`ldapsearch` allows querying the directory. The `-x` option forces simple authentication. Since no username or password is provided, the test attempts an anonymous query. The base `dc=htb,dc=local` is derived from the domain observed in Nmap.

### Verified fact

The query returned domain objects without credentials. This confirms a working `LDAP anonymous bind`.

To reduce noise, relevant fields were extracted:
...
The output revealed a rich domain structure:

- built-in users and groups;
- Exchange accounts and mailboxes;
- `FOREST$` and `EXCH01$` computer accounts;
- Exchange security groups;
- human users (`sebastien`, `santi`, `lucinda`, `andy`, `mark`);
- the `svc-alfresco` account inside `OU=Service Accounts`.
...
The appearance of `svc-alfresco` does not prove a vulnerability by itself, but it does identify a candidate service account. Service accounts are often relevant because they may be configured for integrations, legacy authentication, or less strict policies.

## 5. User cleanup and the first useful error

The first user list was generated from all `sAMAccountName` values in the LDIF:
...
The resulting list was not a clean user list. It included groups and objects with spaces cut by `awk`. For example, `Remote Management Users` could appear reduced to `Remote`.

### Methodological correction

To avoid mixing users, groups and system objects, the query was repeated with a filter for real user objects:
...
A cleaner list was then created:
...
In Active Directory, not every `sAMAccountName` is a human account that is useful for Kerberos. There are mailboxes, internal accounts, computers, groups and system objects. Before using a list as input for Kerberos tools, it should be cleaned with proper LDAP filters.

## 6. svc-alfresco validation and ASREPRoasting

The ASREPRoasting path requires a specific condition: a user account must have Kerberos pre-authentication disabled. In the notes, an attempt was made to generate a candidate file through LDAP, but a sequencing error occurred: `grep` was run against an `asrep_candidates.ldif` file that did not yet exist. That created an empty `asrep_candidates.txt`.

### Why that error matters

An empty list does not produce a negative test. If `GetNPUsers` is run with a file that contains no users, the candidate account is not really being checked. In this case, the lack of output did not mean that `svc-alfresco` was not vulnerable; it meant that the tool received no valid user.

### Practical correction

Since `svc-alfresco` had already appeared as an object under `OU=Service Accounts`, a minimal list with that account was created:

```bash
printf 'svc-alfresco\n' | tee content/users/asrep_candidates.txt
```

The AS-REP was then requested with Impacket:
...
Kerberos returned an AS-REP hash for the account:
...
This confirms that `svc-alfresco` does not require Kerberos pre-authentication. ASREPRoasting does not require knowing the password in advance; it requests encrypted material that can be attacked offline. The value of the finding is that the later attack does not generate login attempts against the domain.

## 7. Offline cracking of the AS-REP

The obtained hash was attacked offline with John:
...
The password was first validated against SMB:

```bash
netexec smb 10.129.95.210 -u svc-alfresco -p 's3rvice'
```

The output confirmed valid authentication:

```text
[+] htb.local\svc-alfresco:s3rvice
```

It was then validated against WinRM:
...
The credential is not only valid for the domain; it also allows opening a remote session via WinRM. This changes the active branch: it stops being a purely Kerberos phase and becomes an initial foothold phase.

## 8. Initial WinRM access and user.txt retrieval

The remote session was opened with Evil-WinRM:

```bash
evil-winrm -i 10.129.95.210 -u 'svc-alfresco' -p 's3rvice'
```

Inside the session, the context was verified:
...
The output of `whoami /groups` was more important than the flag itself. It showed that `svc-alfresco` belonged to:
...
`Remote Management Users` explains WinRM access. `Service Accounts` explains the functional context of the account. The critical item is `Account Operators`, because this group can create and modify accounts and add users to certain non-protected groups. Escalation stops being a classic local search and becomes an Active Directory permissions problem.

## 9. SharpHound attempt and switch to bloodhound-python

The next phase required analyzing domain relationships. First, `SharpHound.exe` was uploaded and run from the WinRM session:
...
The upload worked, but execution failed due to .NET incompatibility:
...
There was no need to insist on that SharpHound version. Collection could be done from Parrot with `bloodhound-python`, avoiding dependence on the victim machine's .NET runtime.
...
The main JSON files were generated:
...
During collection, this appeared:

```text
KRB_AP_ERR_SKEW(Clock skew too great)
```

This indicates a time skew that can affect Kerberos. However, the collection obtained enough LDAP data to analyze users, groups, OUs and ACLs.

## 10. Targeted BloodHound review with jq

A broad `grep` over the JSON files was too noisy. BloodHound JSON files are not meant to be read raw. More targeted `jq` queries were used instead.
...
`dontreqpreauth: true` retrospectively confirms why ASREPRoasting worked. `hasspn: false` indicates that this account was not a direct Kerberoasting candidate through SPN. The `Aces` block of the `svc-alfresco` object must not be confused with permissions that `svc-alfresco` has over other objects: they are ACLs over the user object itself.

## 11. Confirmation of WriteDacl over the domain

The likely path went through `Exchange Windows Permissions`, so its SID was extracted:
...
The domain ACLs were then checked:
...
The domain object was:

```text
DC=HTB,DC=LOCAL
```

and was marked as a high-value target.

### Implication

The critical relationship is confirmed:

```text
EXCHANGE WINDOWS PERMISSIONS@HTB.LOCAL -- WriteDacl --> HTB.LOCAL
```

This makes it possible to build an escalation path without directly adding an account to `Domain Admins`. The idea is to abuse `WriteDacl` over the domain to grant DCSync-style replication permissions to a controlled account.

## 12. Creating a controlled account

To avoid altering existing users, a new domain account called `r4mforest` was created.

### First attempt and correction

The first attempt used a password longer than 14 characters. `net user` raised an interactive compatibility question for pre-Windows 2000 systems, and Evil-WinRM did not handle the response well. The user was not created.

The correction was to use a complex password of 14 characters or fewer:

```text
R4mForest!26
```

### Creation and memberships

The commands were executed inside Evil-WinRM as `svc-alfresco`:
...
The account was created and active. The observed memberships were:
...
This confirms that `svc-alfresco`, through its `Account Operators` membership, can create users and modify non-protected groups. `Exchange Windows Permissions` is not marked as a protected group, so the controlled account could be added to it.

The path is now prepared:

```text
r4mforest
-> member of Exchange Windows Permissions
-> group with WriteDacl over HTB.LOCAL
-> ability to grant DCSync permissions
```

## 13. Granting DCSync with impacket-dacledit

Initially, PowerView and `Add-ObjectACL` were considered, but `PowerView.ps1` was not available in the tools folder. Instead of blocking the phase, `impacket-dacledit` was used from Parrot.

### Why dacledit makes sense

PowerView and `impacket-dacledit` can both be used to modify ACLs. The practical difference is where they run:

```text
PowerView          -> runs inside Windows, via WinRM
impacket-dacledit  -> runs from Parrot against LDAP/AD
```

Since `r4mforest` was already inside `Exchange Windows Permissions`, and that group had `WriteDacl` over the domain, `dacledit` was a direct and clean path.
...
This means that the domain ACL was modified and that `r4mforest` received enough replication permissions to attempt DCSync.

## 14. Extracting the Administrator hash with secretsdump

With DCSync permissions applied, `secretsdump` was executed while limiting extraction to the `Administrator` user:
...
`secretsdump` used DRSUAPI to obtain `NTDS.DIT` secrets:

```text
[*] Using the DRSUAPI method to get NTDS.DIT secrets
```

The `Administrator` NTLM hash was obtained:

```text
32693b11e6aa90eb43d32c72a07ceea6
```

Kerberos AES and DES keys for the administrator user were also obtained.

### Interpretation

DCSync is not about cracking the administrator password. It abuses domain replication permissions to request directory secrets. The impact is critical: if an account can replicate secrets, it can obtain hashes of privileged accounts.

## 15. Administrative access through pass-the-hash

It was not necessary to crack the `Administrator` hash. In Windows, the NTLM hash can be reused directly in certain protocols through pass-the-hash.
...
Administrative compromise is confirmed. The machine is solved.

## 16. Final technical chain

```text
1. Nmap identifies a Windows Server 2016 Domain Controller.
2. LDAP anonymous bind allows domain object enumeration.
3. Enumeration reveals svc-alfresco in OU=Service Accounts.
4. Kerberos returns an AS-REP for svc-alfresco.
5. John cracks the hash and recovers s3rvice.
6. SMB and WinRM validate the credential.
7. Evil-WinRM allows access as htb\svc-alfresco.
8. whoami /groups reveals Account Operators.
9. BloodHound confirms relevant users, groups and relationships.
10. Exchange Windows Permissions has WriteDacl over HTB.LOCAL.
11. svc-alfresco creates r4mforest and adds it to Exchange Windows Permissions.
12. impacket-dacledit grants DCSync to r4mforest.
13. secretsdump extracts the Administrator NTLM hash.
14. Evil-WinRM uses pass-the-hash with Administrator.
15. root.txt is obtained.
```

## 17. Reusable lessons

### 17.1. A DC is recognized through a set of signals, not by one port alone

`88/tcp`, `389/tcp`, `445/tcp`, `3268/tcp`, `3269/tcp`, `9389/tcp`, a visible domain and the host FQDN form much stronger evidence than any individual port. Correct surface classification avoids wasting time on false web branches.

### 17.2. LDAP anonymous bind can be enough to start a full chain

In Forest, LDAP allowed reading the domain structure, users, OUs, Exchange objects and service accounts. That enumeration is not direct exploitation, but it provides the information needed to decide the next technique.

### 17.3. User lists must be cleaned

Extracting all `sAMAccountName` values without filtering can produce lists contaminated with groups, computers and internal objects. Before using Kerberos or password attack tools, filtering for real users is advisable.

### 17.4. An empty list is not a negative test

The failed attempt with an empty `asrep_candidates.txt` did not demonstrate that `svc-alfresco` was not vulnerable. It only showed that the tool received no users. Documenting this type of error prevents valid hypotheses from being discarded because of sequencing mistakes.

### 17.5. ASREPRoasting confirms a Kerberos misconfiguration

`dontreqpreauth: true` allows encrypted material to be requested without knowing a password beforehand. The relevant attack happens offline during hash cracking.

### 17.6. The user flag is not the most important finding

The real pivot was `whoami /groups`. Membership in `Account Operators` allowed moving from initial access to a domain escalation path.

### 17.7. BloodHound is not analyzed with broad grep

BloodHound JSON files contain many nested relationships. For quick checks, `jq` is more precise. For complete paths, the BloodHound visual interface remains the most suitable tool.

### 17.8. WriteDacl over the domain is a critical primitive

`Exchange Windows Permissions` was not a Domain Admins group, but it had `WriteDacl` over `HTB.LOCAL`. That relationship allowed DCSync permissions to be granted to a controlled account.

### 17.9. DCSync changes the impact of the intrusion

An account with DCSync permissions can request domain secrets. By obtaining the `Administrator` hash, the compromise moves from delegated-privilege user to effective administrative control.

### 17.10. Pass-the-hash avoids depending on the cleartext password

The `Administrator` NTLM hash was enough to open WinRM as administrator. Cracking the password was not required.

## 18. Main evidence

| Phase | Evidence |
|---|---|
| Classification | `FOREST.htb.local`, `htb.local`, Windows Server 2016, AD ports |
| LDAP | Anonymous query returns domain objects |
| Candidate account | `CN=svc-alfresco,OU=Service Accounts,DC=htb,DC=local` |
| ASREPRoasting | `$krb5asrep$23$svc-alfresco@HTB.LOCAL:...` |
| Cracking | `svc-alfresco:s3rvice` |
| Validation | `netexec winrm ... (Pwn3d!)` |
| Foothold | `htb\svc-alfresco` through Evil-WinRM |
| User flag | `789fa35a3fbdddb31ceee14fa4bdc109` |
| Pivot | `BUILTIN\Account Operators` |
| BloodHound | `EXCHANGE WINDOWS PERMISSIONS@HTB.LOCAL` exists |
| Critical ACL | `WriteDacl` over `HTB.LOCAL` |
| Controlled account | `r4mforest:R4mForest!26` |
| DCSync | `DACL modified successfully!` and `secretsdump` through DRSUAPI |
| Administrator hash | `32693b11e6aa90eb43d32c72a07ceea6` |
| Admin | `htb\administrator` through Evil-WinRM |
| Root flag | `2a7cbb6f92926190213605fa2ba841d3` |

## 19. Ambiguous points and editorial corrections applied

The technical chain of the lab has not been changed. The documented resolution matches the evidence observed in the notes.

Editorial corrections applied in the main body:

- normalization of obvious typos such as `nuestrro`, `escript`, `entrorno` or inconsistent uses of `Ip`;
- reorganization of repetitive blocks to avoid a mechanical objective, method and answer template in every microphase;
- reduction of excessively long outputs in the main body while preserving the relevant parts;
- preservation of the operational error with the empty ASREP list as methodological learning;
- preservation of the reconnection and IP change as a real part of the process;
- replacement of the PowerView path with `impacket-dacledit` only because it was the route actually used when `PowerView.ps1` was not present in `tools`.

No new validations or steps have been invented. When an output was interpreted, it was done from evidence recorded in the notes.

## 20. Annex A — Original notes preserved

The following block preserves the original working notes as process traceability. They are kept even when they contain repetitions, long outputs, intermediate decisions or sequence errors, because they are part of the real lab path.
